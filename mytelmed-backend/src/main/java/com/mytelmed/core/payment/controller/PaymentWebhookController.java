...
public class PaymentWebhookController {
  ...
  public ResponseEntity<String> handleStripeWebhook(@RequestBody String payload,
    ...
    if (endpointSecret == null || endpointSecret.isEmpty()) {
      log.warn("Stripe webhook endpoint secret not configured, skipping signature verification");
      return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Webhook endpoint secret not configured");
    }
