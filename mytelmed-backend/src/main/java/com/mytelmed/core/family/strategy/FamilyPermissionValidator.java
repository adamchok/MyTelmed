...
public interface FamilyPermissionValidator {
     ...
     * @param permissionType The required permission type (e.g., "VIEW_MEDICAL_RECORDS", "MANAGE_APPOINTMENTS")
    ...
    /**
     * Gets all patient IDs that the account is authorized to access
     * - If the account is a patient, returns their own ID
     * - If the account is a family member, returns all associated patient IDs
     *
     * @param account The account
     * @return List of patient IDs the account can access (empty list if none)
     */
    List<UUID> getAuthorizedPatientIds(Account account);
}
