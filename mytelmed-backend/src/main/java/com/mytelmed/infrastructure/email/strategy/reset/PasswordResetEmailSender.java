...
@Slf4j
@Component
public class PasswordResetEmailSender implements EmailSenderStrategy {
    ...
    public void sendEmail(String to, Map<String, Object> variables) {
        ...
        String content = templateEngine.process("reset/password-reset", context);

        Message message = Message.builder()
                .from("noreply@" + mailGunDomain)
                .to(to)
                .subject("MyTelmed Account Password Reset")
                .html(content)
                .build();
        ...
        try {
            ...
            try (Response response = feignResponse.join()) {
                if (response.status() < 200 || response.status() >= 300) {
                    log.error("Failed to send password reset email to: {}", to);
            ...
            log.error("Completion error occurred while sending password reset email to {}", to, ce);
            throw new EmailSendingException("Failed to send password reset email.");
        } catch (Exception e) {
            log.error("Unexpected error occurred while sending password reset email to {}", to, e);
            throw new EmailSendingException("Failed to send password reset email");
        }
    }
}
