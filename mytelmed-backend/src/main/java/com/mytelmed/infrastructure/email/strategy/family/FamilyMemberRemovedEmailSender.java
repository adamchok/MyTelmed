...
@Slf4j
@Component
public class FamilyMemberRemovedEmailSender implements EmailSenderStrategy {
    private final MailgunMessagesApi mailgunApi;
    private final SpringTemplateEngine templateEngine;
    private final String mailGunDomain;

    public FamilyMemberRemovedEmailSender(
    ...
    public EmailType getEmailType() {
        return EmailType.FAMILY_REMOVAL;
    }
    ...
    public void sendEmail(String to, Map<String, Object> variables) {
        ...
        String content = templateEngine.process("family/family-member-remove", context);

        Message message = Message.builder()
                .from("noreply@" + mailGunDomain)
                .to(to)
                .subject("MyTelmed Family Member Access Removed")
                .html(content)
                .build();
        ...
        try {
            ...
            try (Response response = feignResponse.join()) {
                if (response.status() < 200 || response.status() >= 300) {
                    log.error("Failed to send family member removed email to: {}", to);
                    throw new EmailSendingException("Failed to send family member removed email");
                }
                log.info("Successfully sent family member removed member email to: {}", to);
            }
        } catch (CompletionException ce) {
            log.error("Completion error occurred while sending family member removed email to {}", to, ce);
            throw new EmailSendingException("Failed to send family member removed email.");
        } catch (Exception e) {
            log.error("Unexpected error occurred while sending family member removed email to {}", to, e);
            throw new EmailSendingException("Failed to send family member removed email");
        }
    }
}
