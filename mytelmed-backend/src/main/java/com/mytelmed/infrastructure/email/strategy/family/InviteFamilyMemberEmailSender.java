...
@Slf4j
@Component
public class InviteFamilyMemberEmailSender implements EmailSenderStrategy {
    private final MailgunMessagesApi mailgunApi;
    private final SpringTemplateEngine templateEngine;
    private final String mailGunDomain;

    public InviteFamilyMemberEmailSender(
    ...
    public EmailType getEmailType() {
        return EmailType.FAMILY_INVITATION;
    }
    ...
    public void sendEmail(String to, Map<String, Object> variables) {
        ...
        String content = templateEngine.process("family/family-member-invite", context);

        Message message = Message.builder()
                .from("noreply@" + mailGunDomain)
                .to(to)
                .subject("MyTelmed Family Member Invite")
                .html(content)
                .build();
        ...
        try {
            ...
            try (Response response = feignResponse.join()) {
                if (response.status() < 200 || response.status() >= 300) {
                    ...
                    throw new EmailSendingException("Failed to send family member invitation email");
                }
                log.info("Successfully sent family member invitation email to: {}", to);
            }
        } catch (CompletionException ce) {
            log.error("Completion error occurred while sending family member invitation email to {}", to, ce);
            throw new EmailSendingException("Failed to send family member invitation email.");
        } catch (Exception e) {
            log.error("Unexpected error occurred while sending family member invitation email to {}", to, e);
            throw new EmailSendingException("Failed to send family member invitation email");
        }
    }
}
