...
@Slf4j
@Component
public class EmailVerificationEmailSender extends BaseEmailSenderStrategy {
    ...
    protected String buildSubject(Map<String, Object> variables) {
        return "MyTelmed - Verify Your Email Address";
    }

    @Override
    protected void validateRequiredVariables(Map<String, Object> variables) {
        String token = (String) variables.get("verificationToken");
        String uiHost = (String) variables.get("uiHost");

        if (!StringUtils.hasText(token)) {
            throw new IllegalArgumentException("Verification token is required and cannot be empty");
        }
        ...
        log.debug("Validation passed for email verification email with token: {}", token);
    }
}
