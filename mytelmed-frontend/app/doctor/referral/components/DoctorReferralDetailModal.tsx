...
import { Modal, Typography, Descriptions, Tag, Button, Divider, Avatar, Badge, Alert, Popconfirm } from "antd";
import {
    Calendar,
    Clock,
    User,
...
interface DoctorReferralDetailModalProps {
...
const DoctorReferralDetailModal: React.FC<DoctorReferralDetailModalProps> = ({
    referral,
    isVisible,
    onClose,
    onAccept,
    ...
    const handleViewAppointment = () => {
        if (referral.scheduledAppointment) {
            router.push(`/doctor/appointment/${referral.scheduledAppointment.id}`);
        }
    };

    // Get modal title based on direction
    const getModalTitle = () => {
        if (isIncoming) {
            return "Incoming Referral Details";
        }
        return "Outgoing Referral Details";
    };

    // Generate footer buttons based on context
    const getFooterButtons = () => {
        ...
        if (isIncoming) {
            // For incoming referrals, show doctor action buttons
            if (referral.status === ReferralStatus.PENDING) {
                ...
                buttons.unshift(
                    <Popconfirm
                        ...
                        onConfirm={() => onAccept?.(referral.id)}
                        okText="Accept"
                        cancelText="Cancel"
                        okButtonProps={{
        ...
        if (referral.status === ReferralStatus.SCHEDULED && referral.scheduledAppointment) {
            buttons.unshift(
                <Button
                    key="appointment"
                    type="primary"
    ...
    return (
        ...
        >
            <div className="py-2">
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-6 gap-3">
                    <div className="flex items-center gap-2">
                        ...
                        <Badge
                            ...
                            style={{
                                backgroundColor:
                                    referral.referralType === ReferralType.EXTERNAL ? "#7c3aed" : "#22c55e",
                                fontSize: "12px",
                            }}
                        />
                    </div>
                ...
                <Descriptions bordered column={{ xxl: 2, xl: 2, lg: 2, md: 1, sm: 1, xs: 1 }} size="small" className="mb-4">
                    <Descriptions.Item label="Patient Name" span={2}>
                        <div className="flex items-center gap-2">
                            ...
                            />
                            <div>
                                <Text strong>{referral.patient.name}</Text>
                                <br />
                                <Text type="secondary" className="text-sm">
                                    {referral.patient.gender} • DOB: {referral.patient.dateOfBirth}
                                </Text>
                            </div>
                        </div>
                    </Descriptions.Item>
                    ...
                    {referral.patient.phone && (
                        <Descriptions.Item label="Phone">
                            <div className="flex items-center gap-2">
                                ...
                                <Text>{referral.patient.phone}</Text>
                            </div>
                        </Descriptions.Item>
                    )}

                    <Descriptions.Item label="Referral Number">
                ...
                {referral.status === ReferralStatus.COMPLETED && (
                    <>
                        ...
                        <Alert
                            ...
                            description="This referral has been completed successfully."
                            type="success"
                            showIcon
                        />
                    </>
                ...
                {referral.status === ReferralStatus.EXPIRED && (
                    <>
                        ...
                        <Alert
                            ...
                            description="This referral has expired and can no longer be used."
                            type="warning"
                            showIcon
                        />
                    </>
...
export default DoctorReferralDetailModal; 