...
} from "../api/pushNotification/props";
...
export function usePushNotifications(): UsePushNotificationReturn {
  ...
  const checkSubscriptionStatus = useCallback(async () => {
    ...
    try {
      const currentSubscription =
        await PushNotificationAPI.getCurrentSubscription();
  ...
  const requestPermission = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage =
        err.message || "Failed to request notification permission";
  ...
  const subscribe = useCallback(
    async (deviceInfo?: string): Promise<boolean> => {
      ...
      try {
        ...
        const response = await PushNotificationAPI.subscribeWithPermission(
          deviceInfo
        );
