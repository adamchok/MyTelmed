...
import { PushSubscriptionRequest, PushSubscriptionResponse, PushNotificationPermission } from "./props";
...
class PushNotificationAPI {
     ...
     */
    static async subscribe(subscription: PushSubscription, deviceInfo?: string): Promise<PushSubscriptionResponse> {
        try {
            const request: PushSubscriptionRequest = {
                endpoint: subscription.endpoint,
                p256dh: PushNotificationAPI.arrayBufferToBase64(subscription.getKey("p256dh")),
                auth: PushNotificationAPI.arrayBufferToBase64(subscription.getKey("auth")),
                userAgent: typeof navigator !== "undefined" ? navigator.userAgent : "",
                deviceInfo: deviceInfo || PushNotificationAPI.getDeviceInfo(),
            };

            const response = await repository.post("/push-subscriptions/subscribe", request);
            ...
            return {
                success: true,
                message: response.data?.message || "Successfully subscribed to push notifications",
     ...
     */
    static async unsubscribe(endpoint: string): Promise<{ success: boolean; message: string }> {
        try {
            const response = await repository.delete("/push-subscriptions/unsubscribe", {
                params: { endpoint },
            });
            ...
            return {
                success: true,
                message: response.data?.message || "Successfully unsubscribed from push notifications",
            };
        } catch (error: any) {
    ...
    static async unsubscribeAll(): Promise<{
        ...
        try {
            const response = await repository.delete("/push-subscriptions/unsubscribe/all");
            ...
            return {
                success: true,
                message: response.data?.message || "Successfully unsubscribed from all push notifications",
            };
        } catch (error: any) {
            console.error("Failed to unsubscribe from all push notifications:", error);
            throw new Error(error.response?.data?.message || "Failed to unsubscribe from all push notifications");
        }
    }
     ...
     * @returns Promise<PushSubscriptionResponse>
     */
    static async subscribeWithPermission(deviceInfo?: string): Promise<PushSubscriptionResponse> {
        try {
            ...
            const subscription = await registration.pushManager.subscribe(PushNotificationAPI.getNotificationOptions());
    ...
    private static urlBase64ToUint8Array(base64String: string): Uint8Array {
        ...
        if (typeof window === "undefined") {
            // Return empty array for SSR - this should not be called during SSR anyway
