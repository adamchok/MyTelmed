...
// Parse Java LocalDateTime array to Dayjs object
// Java LocalDateTime serializes as [year, month, day, hour, minute, second, nano]
// where month is 1-based (January = 1, February = 2, etc.)
export const parseLocalDateTime = (localDateTime: any): Dayjs => {
    if (!localDateTime) {
        return dayjs();
    }
    ...
    if (typeof localDateTime === "string") {
        return dayjs(localDateTime);
    }

    // If it's an array (Java LocalDateTime format)
    if (Array.isArray(localDateTime)) {
        const [year, month, day, hour = 0, minute = 0, second = 0] = localDateTime;
        // Note: month in LocalDateTime is 1-based, but dayjs month is 0-based
        return dayjs()
            .year(year)
            .month(month - 1)
            .date(day)
